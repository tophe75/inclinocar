...
// ── Axis math — StickS3 BMI270, verified empirically 3-position test ──
// Mounted orientation (display face-left): gravity on +Y
// Signs match StickC Plus wire convention (both negated for app bubble)
float computeRawPitch(float ax, float ay, float az) {
  return -atan2(ax, ay) * 180.0/PI;
}
float computeRawRoll(float ax, float ay, float az) {
...
void seedFilters() {
  ...
  float p = computeRawPitch(ax, ay, az) - pitchOffset;
  float r = computeRawRoll(ax, ay, az)  - rollOffset;
...
void calibrate() {
  ...
  for (int i = 0; i < 50; i++) {
    float ax, ay, az;
    M5.Imu.getAccelData(&ax, &ay, &az);
    pSum += computeRawPitch(ax, ay, az);
    rSum += computeRawRoll(ax, ay, az);
    delay(20);
  }
...
void setup() {
  M5.begin();
  Serial.begin(115200);
  Serial.setTxTimeoutMs(0);  // Non-blocking USB CDC — prevents WDT boot loop when port is unread
  Serial.println("InclinoCore StickS3 starting...");
...
void loop() {
  ...
  float rotPitch = computeRawPitch(ax, ay, az) - pitchOffset;
  float rotRoll  = computeRawRoll(ax, ay, az)  - rollOffset;
  ...
  float pitch = medianOf(pitchBuf);
  float roll  = medianOf(rollBuf);
